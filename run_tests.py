...
if __name__ == "__main__":
    ...
    scheduler.add_job(delete_old_screenshots, 'cron', hour=7, minute=0, args=[Config.SCREENPATH])
    # 添加定时任务，设置为每天凌晨3点执行删除过期日志的任务，可根据实际需求调整触发时间
    scheduler.add_job(delete_old_logs, 'cron', hour=7, minute=0, args=[Config.LOG_PATH])
